...
namespace NetworkCheckers.Jogo.Engine
{
    class CheckerMoveRules
    {
        ...
        {
            for (int pos = 1; pos <= BoardConstants.SquareCount; pos++)
            {
                Piece piece = board[pos];
                Location location = Location.FromNotation(pos);
        ...
        private static bool GetWalks(ICollection<Move> moves, IBoard board, int row, int col, int verticalDirection, int horizontalDirection)
        {
            ...
            {
                //space is empty
                moves.Add(new Move(Location.Notation(row, col), Location.Notation(newRow, newCol)));
            }
        ...
        private static MoveStatus PerformMove(IBoard board, int startRow, int startCol, int endRow, int endCol)
        {
            ...
            if (moveStatus != MoveStatus.Illegal)
            {
                ...
                if ((board[endRow, endCol] == Piece.Black) && (endRow == BoardConstants.Rows - 1))
                {
                    board[endRow, endCol] = Piece.BlackKing;
                }
                else if ((board[endRow, endCol] == Piece.White) && (endRow == 0))
                {
